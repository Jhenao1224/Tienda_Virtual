    ...
    if(getData.length > 0){
        for(var i=tab_start; i<tab_end; i++){
            ...
            if(staff){
                ...
                <td>${staff.Nombre}</td>
                <td>${staff.DescripcionVal}</td>
                <td>${staff.PrecioVal}</td>
                <td>${staff.CantidadVal}</td>
                <td>${staff.FechaVal}</td>
               
                <td>
                    <button onclick="readInfo('${staff.picture}', '${staff.Nombre}', '${staff.DescripcionVal}', '${staff.PrecioVal}', '${staff.CantidadVal}', '${staff.FechaVal}')"><i class="fa-regular fa-eye"></i></button>

                    <button onclick="editInfo('${i}', '${staff.picture}', '${staff.Nombre}',  '${staff.DescripcionVal}', '${staff.PrecioVal}', '${staff.CantidadVal}', '${staff.FechaVal}')"><i class="fa-regular fa-pen-to-square"></i></button>
...
function readInfo(pic, nombre, descripcion, precio, cantidad, fecha){
    imgInput.src = pic
    Nombre.value = nombre
...
function editInfo(id, pic, nombre, descripcion, precio, cantidad, fecha){
    isEdit = true
    editId = id
...
form.addEventListener('submit', (e)=> {
    ...
    const information = {
        id: Date.now(),
        picture: imgInput.src == undefined ? "./img/images.png" :imgInput.src,
        Nombre: Nombre.value,
...
filterData.addEventListener("input", ()=> {
    ...
    if(searchTerm !== ""){
        ...
        const filteredData = originalData.filter((item) => {
            ...
            const Descripcion = item.DescripcionVal.toLowerCase()
            const Precio = item.PrecioVal.toLowerCase()
